---
title: A pit crew gauntlet takes on the reference product
date: "2026-08-04"
time: "20:16:37"
tags:
    - loop
...
height: 480
caption: The loop: builders build, fresh critics judge against the real product, the pit crew turns complaints into instruments
```

The ask: a desktop Electron app with feature parity to the reference product's raster→vector
workflow, built by the pit crew variant of the gauntlet loop we've been prototyping —
Fable orchestrating, Opus subagents doing the work. The bet under test is the blog
post's thesis: the loop's speed limit is measurement, not intelligence, so a third
role that does nothing but build instruments should pay for itself.
...
would have cost more than the parallelism bought. And "make it look like
the reference product" as a critic prompt lost to an acceptance checklist with ids (A1–D4)
plus numeric fidelity gates, because adjectives don't fail CI.
