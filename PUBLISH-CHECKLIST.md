...
- [ ] 0. Pre-flight
- [ ] 1. Replace the artwork fixtures with the fox set
- [ ] 2. Scrub artwork from git history
...
`fixtures/reference/artwork.*` is Nintendo/Game Freak artwork. It was the gold-standard
...
| | artwork set (removing) | fox set (keeping) |
| --- | --- | --- |
| source | `artwork.png` 1046×833, 33% transparent | `fox-sticker.png` 1024×1024, 76.5% transparent |
| opaque variant | — | `fox-sticker-white.png` (white-flattened) |
| primary exemplar | `artwork.svg` — 34 paths, 31 KB, ~16 colours + Smart AA | `fox-sticker-clipart-8colors-smartAA.svg` — **63 paths, 7 groups, 35.5 KB**, Clipart / 8 colours / Smart AA on / Enhance on / min-area 5 px², viewBox `0 0 10240 10240` |
| second exemplar | `artwork-clipart-6colors-min90.svg` — 93 paths, 91 KB, 6 colours / min-area 90 px² / AA off | none — see the note in step 1.2 |
| third exemplar | `artwork-clipart-18colors-min90-smartAA.svg` — 67 paths, 42 KB | none |
...
```bash
git rm fixtures/reference/artwork.png \
       fixtures/reference/artwork.svg \
       fixtures/reference/artwork-clipart-6colors-min90.svg \
       fixtures/reference/artwork-clipart-18colors-min90-smartAA.svg
```

**Also `katra/media/screenshot-1785892659092-2.jpg`.** It is a screenshot of the
the reference product editor with the Artwork artwork loaded in both panes — same problem, different
file type, and it is easy to miss because it does not have "artwork" in its name. Either
...
`fixtures/manifest.json` is **generated** — do not hand-edit it. The two artwork entries are
defined in `scripts/generate-fixtures.mjs` around **lines 500–560**:

- `reference-artwork` (line 503) → rename to `reference-fox`
  - `file: 'reference/artwork.png'` → `'reference/fox-sticker.png'`
  - `exemplar: 'reference/artwork.svg'` → `'reference/fox-sticker-clipart-8colors-smartAA.svg'`
  ...
  - `note:` drop the cross-reference to `reference-artwork-6c`
- `reference-artwork-6c` (line 541) → **delete, or repoint at `fox-sticker-white.png`.**
...
- Keep `maxTransparentAreaColorError: 8`. The fox is 76.5% transparent, so this fixture is a
  *stronger* alpha guard than artwork was, not a weaker one.
...
18 tracked files mention artwork. Work through them:

| file | what to change |
| --- | --- |
| `tests/e2e/helpers.ts:21` | `artwork: join(FIXTURES,'reference','artwork.png')` → `fox: join(FIXTURES,'reference','fox-sticker.png')` |
| `tests/e2e/c-preview-interaction.spec.ts:35,57` | `FIXTURE.artwork` → `FIXTURE.fox` |
...
| `tests/e2e/d5-export-formats.spec.ts:91` | comment naming `artwork.svg` as the fill-notation source → name the fox exemplar |
| `tests/engine/alpha.test.mjs:86,88` | `loadPair('reference/artwork.png')` → `'reference/fox-sticker.png'`; the "33% transparent" comment becomes 76.5% |
| `tests/engine/parity.test.mjs:39,153-155,328` | `load('reference/artwork.png')` → fox; the AA off/smart/mid comparison at `colorCount: 16` should move to `8` to match the exemplar |
| `tests/engine/rendered.test.mjs` (13 refs, incl. 44, 98, 116-118, 163-165, 178, 190-191, 201-202, 210-212) | the exemplar A/B block. `readFileSync(fixture('reference/artwork.svg'))` → the fox exemplar; curve-ratio baseline `0.639` needs re-measuring on the fox exemplar; the 6-colour test at line 210 ("the real product keeps artwork's black outline") needs re-basing on the fox or deleting with the 6c fixture |
| `REFERENCE.md:75-89` | the "Gold-standard exemplar" section — rewrite around the fox, keeping the 3×/5× economy language |
| `docs/HARNESS.md` (5 refs) | fixture table row, the `inkRecall` bar (`≥ 0.94 (artwork)`), the `sourceTransparentRatio` context values (`0.33 for artwork` → `0.765 for the fox`), and the decode-contract war story — that story is worth keeping, just rename the fixture |
| `docs/TESTIDS.md:295` | comment citing `artwork.svg` for the inherited-fill notation → fox exemplar |
...
```bash
git grep -il artwork || echo "clean"
...
git add -A && git commit --no-gpg-sign \
  -m "Retire the artwork exemplar set for the license-clean fox fixtures"
```
...
git filter-repo --force --invert-paths \
  --path fixtures/reference/artwork.png \
  --path fixtures/reference/artwork.svg \
  --path fixtures/reference/artwork-clipart-6colors-min90.svg \
  --path fixtures/reference/artwork-clipart-18colors-min90-smartAA.svg \
...
is the licensing problem. The string "artwork" survives in old commit messages and in old
...
Be aware this edits historical source code (identifiers like `FIXTURE.artwork` become
...
> **This is the step people get wrong.** `origin` already contains the artwork blobs
...
```bash
git log --all --oneline -- '*artwork*'          # (d) must print nothing
git log --all --oneline -- 'katra/media/screenshot-1785892659092-2.jpg'
git grep -i artwork $(git rev-list --all) -- 2>/dev/null | head   # slow but definitive
git rev-list --objects --all | grep -i artwork
```

And a working-tree sweep:

```bash
git grep -il artwork || echo "tree clean"
```
