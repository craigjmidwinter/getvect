...
test('[B3] a palette fed back unchanged repaints nothing — it must not re-segment', async () => {
  ...
  const cases = [
    ...
    ['the gold standard at the defaults', artworkIn, {}],
    ['the gold standard at 16 colours + Enhance', artworkIn, { colorCount: 16, enhance: true }],
  ];
...
test('[B3] a merge survives the next setting change', async () => {
   ...
   */
  const base = await run(artworkIn, { colorCount: 16, enhance: true });
  ...
  const merged = await run(artworkIn, { colorCount: 16, enhance: true, palette: mergeInto0 });
  assert.equal(
  ...
  const again = await run(artworkIn, { colorCount: 16, enhance: true, palette: merged.slots });
  assert.equal(
    again.svg,
    merged.svg,
...
test('[B3] recolouring one swatch repaints that slot and leaves the drawing alone', async () => {
  ...
  for (const [label, image, overrides] of [
    ['logo-flat-512', flat, {}],
    ['the gold standard', artworkIn, { colorCount: 16, enhance: true }],
  ]) {
    const base = await run(image, overrides);
...
test('[B3] the fold that costs the colours is one the user can turn off', async () => {
   ...
   * `artifacts/metrics.json` under `reference-artwork-noenhance`). Either fix
