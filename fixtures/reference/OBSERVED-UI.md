...
`fixtures/reference/snorlax.png` uploaded (processing worked signed-out; per-image
...
- **Enhance image with AI** (Beta) checkbox. Observed effect on snorlax.png: busy
...
## Parameter-response measurements (snorlax.png, 1045×833, Enhance ON)
...
- `snorlax.png` — source raster
- `snorlax.svg` — user-captured real output (34 paths, 31KB). User recalls settings
  were ~16 colors with **Smart anti-aliasing enabled** — consistent with the measured
  Smart-AA profile below.
- `snorlax-clipart-6colors-min90.svg` — DOM-extracted real output at Clipart /
  6-color palette / Minimum Area 90px² / AA Off / Enhance ON (93 paths, 91KB)
- `snorlax-clipart-18colors-min90-smartAA.svg` — DOM-extracted real output at
...
- Same settings with AA Off: 637 paths, 189KB — replicates the snorlax Smart-AA
  finding (~90% path reduction) on a second subject.

These fox files are the go-forward public exemplars; the snorlax set is Nintendo
