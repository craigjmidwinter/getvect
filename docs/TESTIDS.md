...
| `color-count-hint` | label next to it | **Required:** `data-requested` (slider value), `data-actual` (`result.palette.length`), `data-shortfall` ∈ `none` \| `image` \| `settings`, and text naming the actual number. The image often has fewer colours than the slider asks for; the control and the result must not silently disagree — **and the hint must name the right culprit.** `image` = the pre-merge histogram genuinely had fewer distinct clusters than requested (a six-colour logo has six colours however far right you drag). `settings` = the shortfall came from our own fold (`merge-threshold`, or Smart anti-aliasing's near-duplicate halo fold), in which case the text must not blame the image. **The invariant, not a quoted string:** the text names the actual count and attributes the shortfall to whichever of the two actually caused it, and `data-shortfall` says the same thing. (An example rather than a contract, so it cannot go stale again: at the time of writing the gold standard at 16 colours with the merge threshold at 5 % renders "6 colours in the result — 16 were found and the cleanup settings merged the rest" with `data-shortfall="settings"`, while the same image at the default 0 % returns 11 and still reads `settings` — the other five are Smart anti-aliasing's halo fold, which is also a setting. An earlier revision of this table quoted "10 colours in the result — the image has no more to give" for the same case — the wrong culprit and the wrong number, i.e. exactly the wording this row forbids, written down as the example to copy.) |
...
| `palette-auto-button` | button | Optional; present only while the palette has been hand-edited. **Must carry the same button chrome as `palette-merge-button` / `palette-remove-button`** (cursor, padding, and a border or background): it is the only route back from a destructive palette edit, and rendered as bare text beside the PALETTE label it is the one control on the panel that does not read as pressable. Discards the edits and **restores the palette the edit replaced** — the last auto-computed palette *and* the candidate size that produced it. Clearing `settings.palette` alone is not enough: editing a swatch rewrites `colorCount` to the palette length, so a plain recompute lands somewhere new (16 → palette 10 → recolour → Auto → palette 8, with the `16` chip no longer `aria-checked` and no way back to the result the user was looking at). |
...
| `merge-threshold` | `<select>` | Percentage thresholds (the reference product defaults to 5 %). Read as *coverage*: a colour group covering less than this share of the image is merged into its nearest surviving colour. Raising it must not increase the layer count. |
