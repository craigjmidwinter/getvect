...
- `salientRegion` / `salientRegions` — `{ x, y, width, height }` in source pixels, or a list of
  ...
  **A box may opt out of the aggregates** with `aggregate: false`, and then it is judged
  ...
  is 64×38 of almost nothing but outline — a fifth of it is ink, and the *real product's*
...
The shaded character artwork and its real-product captures, as `local-artwork` (16 colours
