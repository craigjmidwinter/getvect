...
- `salientRegion` / `salientRegions` — `{ x, y, width, height }` in source pixels, or a list of
  ...
  **A box may carry its own thresholds.** `salientRegions[].thresholds` accepts
  ...
  on `reference-snorlax-default` the paw-pad bar is 22 because the real product's own
...
| `reference/snorlax.png` | real 1046×833 artwork (**not** generated) | REFERENCE's gold-standard A/B source; instrumented five times, as `reference-snorlax` (16 colours + Enhance, exemplar `reference/snorlax.svg`, regions = the face, the **muzzle** and the paw pad), `reference-snorlax-noenhance` (16 colours, Enhance off; looser ratios, 8× bytes / 12× sub-paths), `reference-snorlax-16c-nomerge` (16 colours + Enhance with `mergeThreshold: 0` — the same request with the colour fold explicitly turned off, see below), `reference-snorlax-6c` (6 colours, exemplar `reference/snorlax-clipart-6colors-min90.svg`, paw-pad region) and `reference-snorlax-default` (**no `settings` key at all**, regions = face / muzzle / paw pad, each with its own bar — whatever `DEFAULT_SETTINGS` says today, on real artwork. The other rows each pin a configuration, which is how the 8-colour default a user actually gets went unmeasured while its output painted the paw pad the wrong hue) |
