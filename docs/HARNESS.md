...
| `reference/artwork.png` | real 1046×833 artwork (**not** generated) | REFERENCE's gold-standard A/B source; instrumented three times, as `reference-artwork` (16 colours + Enhance, exemplar `reference/artwork.svg`, `salientRegion` = the face), `reference-artwork-noenhance` (16 colours, **Enhance off** — the configuration a user gets by default, which every other exemplar gate skipped; looser ratios, 8× bytes / 12× sub-paths) and `reference-artwork-6c` (6 colours, exemplar `reference/artwork-clipart-6colors-min90.svg`) |
