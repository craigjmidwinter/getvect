...
| `tests/e2e/ai-enhance.spec.ts` | **AI Enhance (bring your own key)** — the toggle is inert without a key and live with one · the key never comes back to the renderer (field, DOM, storage, and no `get*` on the bridge) · the privacy notice is rendered text at the switch, not a tooltip · a successful run swaps the working image (the traced `viewBox` becomes the stub's 256×160) and shows "Enhancing with AI…" while it does · a bad key and a dead network each fall back to the un-enhanced image with a specific message · the classical `enhance-toggle` is unaffected. Fully offline: `aiEnhance:run` routes to the deterministic stub in `src/main/aiEnhance.ts` under `GETVECT_E2E=1`, and the key store is a per-test temp dir (`GETVECT_AI_DIR`), never the real one |
