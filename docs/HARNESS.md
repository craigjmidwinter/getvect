...
**Why a fixture row exists just to set `mergeThreshold: 0`.** `reference-artwork` asks for
...
`reference-artwork-noenhance`), so both halves of the row are known-reachable. It fails at
